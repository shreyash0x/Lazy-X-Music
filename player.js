...
function initializePlayer(client) {
    ...
    client.riffy.on("nodeConnect", node => {
        console.log(`${colors.cyan}[ LAVALINK ]${colors.reset} ${colors.green}Node Connected ✅${colors.reset}`);
    });
    
    client.riffy.on("nodeError", (node, error) => {
        console.log(`${colors.cyan}[ LAVALINK ]${colors.reset} ${colors.red}Node Error ❌ | ${error.message}${colors.reset}`);
    });
    ...
    client.riffy.on("trackStart", async (player, track) => {
        ...
        try {
            ...
            .setFooter({ text: `Developed by Lazy X Music | v1.2`, iconURL: musicIcons.heartIcon })
            .setTimestamp()
            .setDescription(  
    ...
    client.riffy.on("queueEnd", async (player) => {
        ...
        try {
            ...
            if (autoplaySetting?.autoplay) {
                ...
                const search = `https://www.youtube.com/watch?v=${previousTrack.info.identifier}&list=RD${previousTrack.info.identifier}`;
                const resolve = await client.riffy.resolve({ query: search, requester: client.user.username });

                if (resolve.loadType === 'playlist' && resolve.tracks.length > 0) {
                    const nextTrack = resolve.tracks[2];

...
async function play(client, interaction, lang) {
    try {
        ...
        try {
            player.connect();
            console.log('[PLAY] Player connect() called.');
        } catch (e) {
            console.error('[PLAY] Error calling player.connect():', e);
        }
        ...
        if (query.includes('spotify.com')) {
            ...
            console.log('[PLAY] Resolving query:', query);
            const resolve = await client.riffy.resolve({ query, requester: interaction.user.username });
            console.log('[PLAY] resolve.loadType =', resolve?.loadType, 'tracks =', resolve?.tracks?.length);
            ...
            if (resolve.loadType === 'playlist') {
                ...
                player.queue.add(track);
                requesters.set(track.info.uri, interaction.user.username);
                console.log('[PLAY] Queued track:', track?.info?.title);
            } else {
        ...
        console.log('[PLAY] Queue size after enqueue =', player.queue.size);
        if (!player.playing && !player.paused) {
            console.log('[PLAY] Calling player.play()');
            player.play();
        } else {
            console.log('[PLAY] Not calling play() because playing=', player.playing, 'paused=', player.paused);
        }
